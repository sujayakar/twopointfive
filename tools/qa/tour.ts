...
//   - stops 0‥11 entered in order, each for its authored duration, and the tour ends on its own at the hand-over stop (handedOver, quietUtility off);
//   - per stop: 3 (takedown) exactly one guard dead by its end, the staged one · 4 (firefight) the two posted men dead, zero hits · 5 (room clear) the
//     conference door pulled to, breached (unlatched) during, back at its authored angle after, ≥ 10 barks, the drill reaching 'resolve', nobody left
//     scripted · 7 (drive pull) a scratch mission that really gets pulled (by F on the marker, not the beat's net), the rack's LEDs going dark, and the
//     player's own mission object / rack lights / server panel put back at exit;
...
// into a guard's awareness during stop 9 — exit 0 only if BOTH were reported (the run itself then FAILs, as it should).
...
if (ABORT && !(ABORT.stop >= 0 && ABORT.stop <= 10 && ABORT.frac >= 0 && ABORT.frac < 1)) { console.error('--abort-at S[:frac]: S in 0‥10 (stopping in the hand-over stop 11 is a normal finish), frac in [0, 1)'); process.exit(2); }
const DT = 1 / HZ;
const MAX_DT = 1 / 20;       // main.ts: dt = min(1/20, raw)
const STOPS = 12;            // the tour's contract: 12 stops, the last one the hand-over
...
async function runOnce(seed: number, runIdx: number): Promise<RunReport> {
  ...
  function perFrame() {
    ...
    acc.deadLast = game.guards.filter((g: any) => g.state === 'dead').length;
    if (i === 3) {   // the beat's milestones on the shot clock (timeline notes; and where an --abort-at 3:frac lands)
    ...
    if (i === 4) { acc.guardShots = (acc.guardShots ?? 0) + game.events.filter((e: any) => e.kind === 'guardShot' && e.time === game.time).length; acc.playerShots = (acc.playerShots ?? 0) + game.events.filter((e: any) => e.kind === 'shot' && e.time === game.time).length; }
    if (i === 5) { if (confDoor && !confDoor.latched) acc.sawUnlatched = true; if (demo.drill) acc.phases.add(demo.drill.phase); }
    if (i === 7) {
    ...
    if (i !== 5 && game.guards.some((g: any) => g.script)) fail(`script-outside-5@${i}`, `a guard carries a script during stop ${i} (${game.guards.filter((g: any) => g.script).map(routeName).join(',')})`);
    if (game.escalation !== 0) fail('escalation-in-tour', `escalation ${game.escalation} during the tour (quietUtility=${game.quietUtility})`);
  }
  ...
  function runLap(abortAt: { stop: number; frac: number } | null) {
    ...
    function enter(i: number, missionBefore: any) {
      ...
      if (i === 3) { acc.tdTarget = game.guards[1 % game.guards.length]; }
      if (i === 4) {
        acc.posted = game.guards.filter((g: any) => g.state !== 'dead' && g.pinned && !g.hold);
        if (acc.posted.length !== 2) fail('stop4-posted', `firefight: expected 2 posted (alert, pinned) guards after enter, found ${acc.posted.length} (${game.guards.map((g: any) => `${routeName(g)}:${g.state}${g.pinned ? '+pinned' : ''}${g.hold ? '+hold' : ''}`).join(' ')})`);
        if (game.playerInvisible) fail('stop4-visible', 'firefight: playerInvisible still on after enter (the beat is meant to be seen)');
        const p = game.player.pistol; if ((p.mag ?? 0) + p.chamber < 6) fail('stop4-kit', `firefight: Sam enters with mag ${p.mag} chamber ${p.chamber} (rearm() should have topped him up)`);
      }
      if (i === 5) {
        acc.sawUnlatched = false; acc.phases = new Set<string>();
        if (!confDoor) fail('stop5-door-missing', "no door named 'conference'"); else if (!confDoor.latched || Math.abs(confDoor.angle) > 1e-3) fail('stop5-door-shut', `room clear: conference door not pulled to at enter (angle ${confDoor.angle.toFixed(2)}, latched ${confDoor.latched})`);
        if (!demo.drill) fail('stop5-drill-null', 'room clear: demo.drill is null after enter (no RoomClear constructed — fewer than 2 guards?)');
      }
      if (i === 6) { if (game.player.canisters < 1) fail('stop6-kit', 'smoke stop: Sam enters with no smoke canister'); }
      if (i === 7) {
        acc.missionBefore = missionBefore; acc.sawPulled = false; acc.sawRackOff = false; acc.sawDriveT = false; acc.rackCheckSent = false;
        if (game.mission === missionBefore) fail('stop7-scratch', 'drive pull: game.mission was not swapped for a scratch copy at enter (the beat would spend the player\'s own run)');
        if (!game.missionInTour) fail('stop7-missionInTour', 'drive pull: missionInTour not raised at enter');
        if (!rackTargets.length) fail('stop7-rack-none', 'no rack targets (game.rackTargets empty)'); else if (!rackTargets.every((t: any) => t.on)) fail('stop7-rack-lit', 'drive pull: rack LEDs not lit at enter');
        if (game.mission.stage !== 'infiltrate' || game.mission.pulled) fail('stop7-scratch-stage', `scratch mission starts at stage ${game.mission.stage} pulled=${game.mission.pulled}`);
      }
      if (i === 11) {   // the hand-over: restartEncounter() has just run inside enter — this is the moment the level must read as authored (before live patrols touch it)
        const bad = levelDiff();
    ...
    function exit(i: number, aborted = false) {
      ...
      if (i === 3) {
        const dead = game.guards.filter((g: any) => g.state === 'dead');
        if ((acc.deadLast ?? 0) !== 1) fail('stop3-takedown', `takedown: ${acc.deadLast ?? 0} guards dead at the end of the stop (expected exactly 1)`);
        else if (dead.length === 1 && dead[0] !== acc.tdTarget) fail('stop3-takedown-who', `takedown: the dead man is ${routeName(dead[0])}, not the staged target ${routeName(acc.tdTarget)}`);
        ...
        if (!acc.grabbedEver) warn('stop 3: the body was never dragged (player.dragging stayed null the whole stop)');
      }
      if (i === 4) {
        const posted: any[] = acc.posted ?? []; const alive = posted.filter((g: any) => g.state !== 'dead');
        if (alive.length) fail('stop4-firefight', `firefight: ${alive.length} of the ${posted.length} posted men still alive at the end (${alive.map((g: any) => `${routeName(g)}:${g.state} dazzled=${game.time < g.dazzledUntil}`).join(' ')}); player at (${f2(game.player.char.pos[0])}, ${f2(game.player.char.pos[2])}) mag ${game.player.pistol.mag} chamber ${game.player.pistol.chamber}`);
        rec?.notes.push(`posted ${posted.map(routeName).join('+')}: ${posted.map((g: any) => g.state).join('/')}; guard rounds ${acc.guardShots ?? 0}, Sam's ${acc.playerShots ?? 0}`);
      }
      if (i === 5) {
        if (!acc.sawUnlatched) fail('stop5-breach', 'room clear: the conference door never came unlatched during the stop (no breach)');
        if (says[5] < 10) fail('stop5-barks', `room clear: only ${says[5]} barks during the stop (expected ≥ 10)`);
        if (!acc.phases?.has('resolve')) fail('stop5-drill-resolve', `room clear: the drill never reached 'resolve' (phases seen: ${[...(acc.phases ?? [])].join('→') || 'none'})`);
        if (confDoor && confAuth && (Math.abs(confDoor.angle - confAuth.angle) > 1e-3 || confDoor.latched !== confAuth.latched || confDoor.locked !== confAuth.locked || confDoor.lockBroken)) fail('stop5-restore', `room clear: conference door not restored at exit — angle ${confDoor.angle.toFixed(3)} (authored ${confAuth.angle.toFixed(3)}) latched=${confDoor.latched} locked=${confDoor.locked} broken=${confDoor.lockBroken}`);
        for (const t of game.targets) if (t.group === 'conference' && t.on !== refLights.get(t)) { fail('stop5-lights', `room clear: ${t.name} left on=${t.on} at exit (authored ${refLights.get(t)})`); break; }
        if (game.guards.some((g: any) => g.script)) fail('stop5-script', `room clear: a guard still carries a script after exit (${game.guards.filter((g: any) => g.script).map(routeName).join(',')})`);
        if (demo.drill) fail('stop5-drill-cleared', 'room clear: demo.drill not cleared at exit');
        rec?.notes.push(`phases ${[...(acc.phases ?? [])].join('→')}; ${says[5]} barks`);
      }
      if (i === 7) {
        if (!acc.sawDriveT) fail('stop7-pull-start', 'drive pull: the pull never started (mission.driveT never went ≥ 0)');
        if (!acc.sawPulled) fail('stop7-pulled', 'drive pull: mission.pulled never became true during the stop');
        if (acc.sawPulled && !acc.sawRackOff) fail('stop7-rack-dark', 'drive pull: the rack LEDs never went dark after the pull');
        if (game.mission !== acc.missionBefore) fail('stop7-restore-mission', `drive pull: the player's own mission object was not put back at exit (stage now ${game.mission.stage}, pulled ${game.mission.pulled})`);
        if (game.missionInTour) fail('stop7-restore-missionInTour', 'drive pull: missionInTour still up after exit');
        const wantOn = !acc.missionBefore?.pulled;
        if (rackTargets.some((t: any) => t.on !== wantOn)) fail('stop7-restore-rack', `drive pull: rack targets on=${rackTargets.map((t: any) => t.on).join('/')} after exit, expected all ${wantOn} (player's mission pulled=${acc.missionBefore?.pulled})`);
        for (const t of game.targets) if (t.group === 'server' && t.on !== refLights.get(t)) { fail('stop7-lights', `drive pull: ${t.name} (server group) left on=${t.on} at exit (authored ${refLights.get(t)})`); break; }
        ...
        if (acc.usedNet) warn(`stop 7: the pull only started at +${f2(acc.pullAt)} s — that is the beat's fallback (m.driveT = 0 past 6 s), the F-on-the-hovered-marker path did not take`);
      }
    ...
    for (; simT < tCap; frame++) {
      ...
      if (SELFTEST) {   // deliberate faults the tripwires must report: a guard put 2.5 m in front of Sam fires one aimed round at his chest; later a guard's awareness goes NaN
        if (!injected.hit && demo.shotI === 1 && demo.shotT >= 3) { injected.hit = true; const gd = game.guards.find((g: any) => g.state !== 'dead'); const pc = game.player.char; const f = pc.forward();
        ...
        if (!injected.nan && demo.shotI === 9 && demo.shotT >= 2) { injected.nan = true; const gd = game.guards[game.guards.length - 1]; if (gd) { gd.awareness = NaN; info(`selftest: NaN put into ${routeName(gd)}'s awareness`); } }
      }
