...
export function buildPanel(panel: Panel, engine: Engine, game: Game, smoke: SmokeSystem, cam: FollowCamera, overlay: Overlay, audio: AudioEngine, quality: Quality) {
  ...
  { const ro = (window as any).__readout as { on: boolean; set(v: boolean): void } | undefined; if (ro) panel.toggleBox(r, 'debug read-out', () => ro.on, v => ro.set(v), 'the top-left GPU pass timings / counts / guard states text (also ?hud); off keeps the corner for the interaction panel'); }
  ...
  // ---------------- direct light
  const dl = panel.section('Direct light', false, true);
  ...
  // ---------------- indirect light
  const il = panel.section('Indirect light', false, true);
  ...
  const rcs = panel.section('Indirect cache (probe volume)', false, true);
  ...
  const ef = panel.section('Effects', false, true);
  ...
  panel.buttons(gp, 'dialogue', [{ text: 'interrogate nearest guard', run: () => game.interrogateNearest(), title: 'one exchange with the living man nearest you (within 2 m — walk up behind one): he answers from his own state — what he personally saw / heard of you, how rattled he is, his temperament — one stage per press (reaction → pressure → the one useful thing he could know, or his refusal → nothing left). The grab that will hold him for this does not exist yet; the line below is the engine\'s read of him (src/game/dialogue.ts — every line a draft placeholder)' }]);
  panel.text(gp, () => game.dialogueSummary());   // 'dialogue ▸ Novak (nervy) · cell none·low·nervy · A 0.15 [bias +0.15] · next press: stage 1 · said 0 · next intel: rollcall · 3.2 m off' (read-only)
  ...
  // ---------------- audio
  const au = panel.section('Audio', false, true);
  ...
  // ---------------- camera
  const c = panel.section('Camera', false, true);
