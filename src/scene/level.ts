...
export function buildLevel(): Level {
  ...
  function monitor(x: number, z: number, yaw: number, on: boolean, baseY = 0.76) {   // baseY = the surface it stands on (desk 0.76, reception counter 1.15)
    const at = frameAt(x, z, yaw);
    ...
    // keyboard
    add({ c: at(0, baseY + 0.01, 0.28), h: [0.2, 0.012, 0.07], yaw, albedo: [0.08, 0.08, 0.09] });
  }
  ...
  // door frames: jambs + head slightly proud of the wall around every doorway that gets a leaf (see `doors` below)
