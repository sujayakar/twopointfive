...
async function main() {
  ...
  function step(dt: number, lastOfBurst = true) {
    ...
    if (!frozen && input.wheel) cam.zoom(input.wheel);
    if (input.down('KeyQ')) cam.rotate(-1.2 * dt);
    if (input.down('KeyE')) cam.rotate(1.2 * dt);
    ...
    if (frameCount % 10 === 0) {
      ...
      hud.textContent = demo.active ? '' : lines.join('\n');   // the showcase keeps the screen clean (captions, meters and kit only)
    }
