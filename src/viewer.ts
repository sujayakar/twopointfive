...
import { Vec3, v3, DEG, clamp, wrapAngle, approachAngle } from './math/vec';
...
import { Character, UpperMode, AnimParams, Stance, SignalKind, KICK_IMPACT, THROW_RELEASE, THROW_DUR, HOLD, HoldPhase, HoldVariant } from './game/character';
...
const STANCES: Stance[] = ['none', 'highReady', 'lowReady', 'stack', 'wall'];
/** the game's sidle along a wall (player.ts WALL_TURN): the legs turn this far toward the direction of travel and the animator is told the wall is that far round
 *  (anim.wallYaw) — emulated here whenever the 'wall' stance is walked, so the torso staying square to a wall behind the treadmill can be judged */
const WALL_TURN = 55 * DEG, WALL_PEEK_LOOK = 30 * DEG;
const TAC_SHOTS = ['kick door', 'signal: hold', 'signal: go', 'signal: rally', 'flinch', 'throw'];
...
async function main() {
  ...
  const st = {
    drive: 0, speedMul: 1, deliberate: 0, upper: 1, aimPitchDeg: 0, aimYawDeg: 0, lookYawDeg: 0,   // deliberate: anim.deliberate (game: threat proximity)
    ...
    track: 1, wall: true,
    stance: 0, stackSide: 1 as -1 | 1, lockpick: false, tacShot: 0, tacLoop: false, tacEvery: 1.5, peek: 0,   // Tactical (procedural) section
    // Hold (grab / human shield) section: ONE mannequin, so it poses either figure of the pair alone — the holder (anim.holdPose) or the held man (anim.heldPose) —
    // with a stick-and-cube marker where the invisible partner's root and skull would be (HOLD.dist ahead of the holder / behind the held man)
    hold: { on: false, who: 0, variant: 1, phase: 1, t: 0, auto: true, gunOut: 0, gunPitchDeg: 0, agitation: 0.4, marker: true, shotT: -1 },
  };
  let tacT = 0; let kickFlashT = 0; let relFlashT = 0; let prevThrowT = -1; let wallTurn = 0;   // wallTurn: the emulated sidle turn now on the body (rad, signed), eased like player.ts's approachAngle
  ...
  panel.slider(a, 'deliberate', 0, 1, 0.01, () => st.deliberate, v => { st.deliberate = v; }, 'anim.deliberate — what the game raises inside ~7 m of a living guard who has not clocked Sam (game.ts threatProximity): crouch walk = cadence × 0.78 with the stride warped longer to match, a linger over each plant, pelvis −4 cm, torso further over with the head level, arms in; walk = arms in + softer knees only. Eased ~0.4 s inside the animator. Independent of it the crouch also sits 3 cm lower and steps 10 % longer at full pace (speed × ≈ 1) than at a creep (≈ 0.4)');
  ...
  const fireTac = (i = st.tacShot) => { const an = char.anim; if (an.rawClip) return; if (i === 0) an.kickDoor(); else if (i === 4) an.flinch(); else if (i === 5) an.throwItem(); else an.signal((['hold', 'go', 'rally'] as SignalKind[])[i - 1]); };
  { const tc = panel.section('Tactical (procedural)');
    panel.select(tc, 'stance', ['none', 'high ready (pieing / covering)', 'low ready (moving)', 'stack (on a door)', 'wall (back to the wall: Q)'], () => st.stance, v => { st.stance = v; }, 'continuous carries layered under aim: compose with the aim yaw offset, walking and crouch; aim / shoot still win. wall = the player\'s Q hold, a body of its own: the wall is taken to be square behind the body-yaw heading (walk it and the legs turn 55° toward "side" the way player.ts sidles, the torso staying on the wall); slot 1 + upper relaxed = pistol up by the cheek, upper none = hands on the wall; try crouch idle / walk / crouch walk and the peek slider. Not aiming, the aim yaw offset is countered (the back stays on the wall, the head does the looking); upper aim brings the chest round with the gun');
    panel.select(tc, 'side (stack: wall on… / wall: looking…)', ['left', 'right'], () => st.stackSide > 0 ? 1 : 0, v => { st.stackSide = v ? 1 : -1; }, 'stack: mirrors the blade — pelvis toward the wall, head to the door. wall: the way along the wall he looks, leads with the foot, carries the gun and peeks (a flip sweeps across, ~0.3 s)');
    panel.slider(tc, 'wall peek', 0, 1, 0.01, () => st.peek, v => { st.peek = v; }, 'anim.wallPeek: leaning out past the wall\'s end on "side" (the game ramps it 0→1 over 0.3 s while you push into an open end); the neck\'s extra 30° crane the game adds (lookYawExtra) is emulated too');
    panel.buttons(tc, 'one-shots', [{ text: 'kick', run: () => fireTac(0), title: 'front kick at handle height; the door should move at the impact instant (flash on the HUD line)' }, { text: 'hold', run: () => fireTac(1) }, { text: 'go', run: () => fireTac(2) }, { text: 'rally', run: () => fireTac(3) }, { text: 'flinch', run: () => fireTac(4) },
      { text: 'throw', run: () => fireTac(5), title: `overhand throw off the right hand, ${THROW_DUR} s, over any locomotion / crouch: the can leaves the hand at THROW_RELEASE = ${THROW_RELEASE} s (flash on the HUD line; with slot 2 the can is drawn in the fist until then). Stills: __tac.at(t, 'throw')` }]);
    ...
    panel.text(tc, () => { const an = char.anim; return `stance ${STANCES[st.stance]}${st.stance >= 3 ? (st.stackSide > 0 ? ' R' : ' L') : ''}${st.stance === 4 ? ` · wall ${Math.round(an.wallYaw / DEG)}° round, peek ${an.wallPeek.toFixed(2)}` : ''} · ${an.kicking ? `kick t=${an.kickTime.toFixed(2)}${an.kickTime >= KICK_IMPACT ? ' (past impact)' : ''}` : an.inThrow ? `throw t=${an.throwTime.toFixed(2)}${an.throwTime >= THROW_RELEASE ? ' (released)' : ' (in hand)'}` : an.signalling ? 'signalling' : an.lockpick ? 'picking' : 'idle'}${an.hideHeldItem ? ' · held item hidden' : ''} · kick impact = ${KICK_IMPACT}s · throw release = ${THROW_RELEASE}s`; });
    ...
    const poseAt = (t: number, shot?: 'kick' | SignalKind | 'flinch' | 'throw') => {
      const an = char.anim; st.paused = true;
      an.stance = STANCES[st.stance]; an.stackSide = st.stackSide; an.lockpick = st.lockpick; an.speed = DRIVES[st.drive].speed(an.params) * st.speedMul; an.crouchTarget = DRIVES[st.drive].crouch; an.upper = UPPERS[st.upper].mode; an.wallPeek = st.peek; an.deliberate = st.deliberate;
      for (let i = 0; i < 400 && (an.kicking || an.signalling || an.inThrow); i++) char.update(1 / 60);
      for (let i = 0; i < 48; i++) char.update(1 / 60);
      if (shot) fireTac(shot === 'kick' ? 0 : shot === 'flinch' ? 4 : shot === 'throw' ? 5 : ['hold', 'go', 'rally'].indexOf(shot) + 1);
      for (let i = 0, n = Math.max(0, Math.round(t * 120)); i < n; i++) char.update(1 / 120);
      return `${shot ?? 'settled'} @ ${t.toFixed(3)} s${an.kicking ? ` (kick t=${an.kickTime.toFixed(3)})` : ''}${an.inThrow ? ` (throw t=${an.throwTime.toFixed(3)}${an.throwTime >= THROW_RELEASE ? ', released' : ', can in hand'})` : ''} — viewer paused`;
    };
    const tac = {   // scripted capture: __tac.stance('stack', -1); __tac.stance('wall', 1); __tac.peek(1); __tac.kick(); __tac.signal('go'); __tac.throw(); __tac.lockpick(true); __tac.at(0.38, 'kick'); __tac.at(0.28, 'throw')
      ...
      peek: (v: number) => { st.peek = clamp(v, 0, 1); },
      kick: () => fireTac(0), signal: (kname: SignalKind) => { const i = ['hold', 'go', 'rally'].indexOf(kname); if (i < 0) throw new Error('signal: hold | go | rally'); fireTac(i + 1); }, flinch: () => fireTac(4), throw: () => fireTac(5),
    ...
    window.__tac = tac;   // (the declared hook type in debugHooks.ts predates throw() / at(t, 'throw'); the object is a superset of it)
  }

  // ---- the grab / hold's two layers (character.ts holdLayer / heldLayer, HOLD): one figure of the pair at a time, the other marked where it would stand
  const HOLD_PHASES: HoldPhase[] = ['grab', 'held', 'choke', 'whip', 'release'];
  const HOLD_VARIANTS: HoldVariant[] = ['arm', 'gun'];
  const holdDur = (ph: HoldPhase) => ph === 'grab' ? HOLD.secs.grab : ph === 'choke' ? HOLD.secs.choke : ph === 'whip' ? HOLD.secs.whip + 0.5 : ph === 'release' ? HOLD.secs.release + 0.3 : 4;   // (a beat past the one-shots' end so the way out shows; 'held' just runs)
  { const hs = panel.section('Hold (grab / human shield)', false); const Hs = st.hold;
    panel.toggleBox(hs, 'pose the hold', () => Hs.on, v => { Hs.on = v; Hs.t = 0; if (v) { st.stance = 0; st.lockpick = false; if (Hs.who === 0 && Hs.variant === 1) { st.slot = 1; st.guard = false; } } }, 'drives anim.holdPose (the holder: Sam) or anim.heldPose (the held man) on this one mannequin — the layer eases itself in and out; upper body / stance are forced off while it is on, the legs keep whatever locomotion is selected (walk it: the hands must stay put). The grey stick + cube mark the invisible partner\'s root and skull (HOLD.dist ahead of the holder, behind the held man)');
    panel.select(hs, 'figure', ['holder (Sam)', 'held man (guard kit: G)'], () => Hs.who, v => { Hs.who = v; Hs.t = 0; if (v === 1) st.guard = true; else { st.guard = false; if (Hs.variant === 1) st.slot = 1; } });
    panel.select(hs, 'variant', ['arm (both hands on him, pistol on the thigh)', 'gun (Five-seveN up beside his head)'], () => Hs.variant, v => { Hs.variant = v; Hs.t = 0; if (v === 1 && Hs.who === 0) st.slot = 1; }, 'fixed at the grab in the game by what is in the hand: drawn → gun, holstered / canister → arm');
    panel.select(hs, 'phase', ['grab (the reach, 0.45 s)', 'held', 'choke (arm: 2.6 s)', 'whip (gun: lands at 0.6 s)', 'release (the shove, 0.35 s)'], () => Hs.phase, v => { Hs.phase = v; Hs.t = 0; });
    panel.slider(hs, 't (s into the phase)', 0, 4, 0.01, () => Hs.t, v => { Hs.t = v; }, 'scrub with auto off (or paused)');
    panel.toggleBox(hs, 'auto-advance t (loops the one-shots)', () => Hs.auto, v => { Hs.auto = v; });
    panel.slider(hs, 'gun out (holder, gun)', 0, 1, 0.01, () => Hs.gunOut, v => { Hs.gunOut = v; }, 'HoldPose.gunOut: 0 = the can along his temple, 1 = presented dead ahead to fire past him (the game eases it up in a few frames on a click and back ~1 s after the last round)');
    panel.slider(hs, 'gun pitch ° (presented)', -45, 45, 1, () => Hs.gunPitchDeg, v => { Hs.gunPitchDeg = v; });
    panel.slider(hs, 'agitation (held man)', 0, 1, 0.01, () => Hs.agitation, v => { Hs.agitation = v; }, 'HeldPose.agitation: the struggle\'s amplitude (the dialogue engine\'s number in the game)');
    panel.buttons(hs, '', [{ text: 'fire past him (K)', run: () => { shoot(); Hs.shotT = 0; }, title: 'holder: the recoil kick on the gun hand (gun variant, gun out); held man: the duck off a round past his ear (HeldPose.shot)' }, { text: 'restart phase', run: () => { Hs.t = 0; } }]);
    panel.toggleBox(hs, 'partner marker', () => Hs.marker, v => { Hs.marker = v; });
    panel.text(hs, () => { const an = char.anim; const P = Hs.who === 0 ? an.holdPose : an.heldPose; return `${Hs.on ? (Hs.who === 0 ? 'holdPose' : 'heldPose') : 'off'} ${P ? `${P.variant ?? 'arm'} · ${P.phase} t ${P.t.toFixed(2)}` : '—'}${an.hideHeldItem ? ' · held item hidden' : ''} · HOLD.dist ${HOLD.dist} · secs ${JSON.stringify(HOLD.secs)}`; });
    window.__hold = { set: (o: Partial<typeof Hs>) => Object.assign(Hs, o), HOLD };   // scripted stills: __hold.set({ on: true, who: 0, variant: 1, phase: 3, auto: false, t: 0.6 })
  }
  ...
  function step(dt: number) {
    ...
    if (canvas.clientWidth > 0 && canvas.clientHeight > 0) {
      ...
      c.anim.speed = speed; c.anim.crouchTarget = D.crouch; c.anim.reverse = D.reverse; c.anim.deliberate = st.deliberate;
      const onWall = STANCES[st.stance] === 'wall' && !c.anim.rawClip; const pk = onWall ? st.peek * st.peek * (3 - 2 * st.peek) : 0;
      c.anim.upper = c.anim.rawClip ? 'none' : U.mode; c.anim.lookYawExtra = st.lookYawDeg * DEG - st.stackSide * WALL_PEEK_LOOK * pk;   // ('aim' would pull gunDir toward the analytic aim while the raw clip's hand is elsewhere)   // (+ the game's peek crane: player.ts moveOnWall)
      c.anim.stance = STANCES[st.stance]; c.anim.stackSide = st.stackSide; c.anim.lockpick = st.lockpick; c.anim.wallPeek = st.peek;
      { // the hold: one figure of the pair (player.ts holdTail sets exactly these fields; upper / stance / crane off as it does)
        const Hs = st.hold; const ph = HOLD_PHASES[Hs.phase], variant = HOLD_VARIANTS[Hs.variant];
        if (Hs.on && !c.anim.rawClip) {
          if (Hs.auto) { Hs.t += cdt; if (ph !== 'held' && Hs.t > holdDur(ph)) Hs.t = 0; }
          if (Hs.shotT >= 0) { Hs.shotT += cdt; if (Hs.shotT > 1.5) Hs.shotT = -1; }
          c.anim.upper = 'none'; c.anim.stance = 'none'; c.anim.lookYawExtra = 0;
          const live = ph === 'held' || Hs.t <= holdDur(ph) - (ph === 'whip' ? 0.5 : ph === 'release' ? 0.3 : 0);   // past a one-shot's end the controller has let go: show the layer's own way out
          c.anim.holdPose = Hs.who === 0 && live ? { phase: ph, t: Hs.t, variant, gunOut: Hs.gunOut, gunPitch: Hs.gunPitchDeg * DEG } : null;
          c.anim.heldPose = Hs.who === 1 && live ? { phase: ph, t: Hs.t, agitation: Hs.agitation, variant, shot: Hs.shotT } : null;
        } else { c.anim.holdPose = null; c.anim.heldPose = null; }
      }
      ...
      const turnWas = wallTurn; wallTurn = approachAngle(wallTurn, onWall && speed > 0.3 ? -st.stackSide * WALL_TURN : 0, 10 * cdt);   // the game's sidle: legs turned toward travel, the wall staying where it was (behind the un-turned heading)
      if (st.path === 1) {   // walk a 2 m circle about the stage centre (analytic, no drift): real foot planting to judge against the floor marks; a backpedal faces against
        ...
        c.bodyYaw = wrapAngle(Math.atan2(-Math.sin(circPhi), Math.cos(circPhi)) + (D.reverse ? Math.PI : 0) + wallTurn); st.bodyYawDeg = Math.round(c.bodyYaw / DEG);
      } else { c.vel = [0, 0, 0]; if (wallTurn !== turnWas) { c.bodyYaw = wrapAngle(c.bodyYaw + wallTurn - turnWas); st.bodyYawDeg = Math.round(c.bodyYaw / DEG); } }
      const awayYaw = c.bodyYaw - wallTurn; c.anim.wallYaw = -wallTurn;   // (the wall's normal off his front)
      c.aimYaw = awayYaw + st.aimYawDeg * DEG; c.aimPitch = st.aimPitchDeg * DEG;   // (on the wall the aim offset reads off the wall's normal, as the game's cursor aim does)
      if (st.hold.on && !c.anim.rawClip) {   // the hold has no chest twist of its own (player.ts updateHold: aimYaw = bodyYaw) and its own pitch
        c.aimYaw = c.bodyYaw;
        c.aimPitch = st.hold.who === 0 ? (HOLD_VARIANTS[st.hold.variant] === 'gun' ? st.hold.gunPitchDeg * DEG * st.hold.gunOut : -0.05)   // (the game presents the gun at the aim's pitch: the drawn prop rides aimDir while a shot plays)
          : 0.55 + 0.25 * Math.sin(time * 1.3 + 2) + 0.1 * Math.sin(time * 3.1);                                                              // (the held man's torch wanders over the ceiling: player.ts holdTail)
      }
      ...
      { const tt = c.anim.throwTime; if (prevThrowT >= 0 && prevThrowT < THROW_RELEASE && tt >= THROW_RELEASE) { relFlashT = 0.25; msg('throw: release (the can leaves the hand here)'); } prevThrowT = tt; relFlashT = Math.max(0, relFlashT - dt); }
      ...
      if (st.guard) {
        ...
        const thrown = c.anim.inThrow && c.anim.throwTime >= THROW_RELEASE;   // the can has left the hand: draw it empty for the rest of the throw
        if (st.showProps) rigBoxes(c, { goggles: st.goggles, nv: st.nv, slot: handsBusy ? 0 : st.slot, lightOn: st.lightOn, isGuard: false, holster: handsBusy || c.alive || st.slot !== 1, held: handsBusy || thrown ? null : st.slot === 2 ? 'smoke' : st.slot === 3 ? 'flash' : null }, dyn, RIG, meta);   // (as game.ts: can up → gun on the thigh + can in the fist; picking → empty hands, gun on the thigh)
      }
      ...
      if (st.hold.on && st.hold.marker) {   // the invisible partner: a stick at his root (HOLD.dist ahead of the holder / behind the held man on the body's facing) and a cube for his skull
        const fw = c.forward(); const sgn = st.hold.who === 0 ? 1 : -1; const root = v3.mad(c.pos, fw, sgn * HOLD.dist);
        const grey: Vec3 = [0.35, 0.36, 0.38];
        dyn.push(makeBox({ c: v3.add(root, [0, 0.72, 0]), h: [0.012, 0.72, 0.012], yaw: c.bodyYaw, albedo: grey, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));
        dyn.push(makeBox({ c: v3.add(root, [0, 1.61, -0.005]), h: [0.075, 0.095, 0.09], yaw: c.bodyYaw, albedo: grey, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));   // (skull centre ≈ 1.61 up, 15 wide × 19 tall × 18 deep — where the standing man's head measures in the held pose)
        dyn.push(makeBox({ c: v3.add(root, [0, 1.43, 0]), h: [0.05, 0.06, 0.05], yaw: c.bodyYaw, albedo: grey, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));         // (neck)
        dyn.push(makeBox({ c: v3.add(root, [0, 1.25, -0.02]), h: [0.17, 0.12, 0.09], yaw: c.bodyYaw, albedo: grey, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));     // (upper chest / shoulders)
      }
    ...
    if (frameCount % 10 === 0) {
      ...
      const animLine = r ? `raw clip ${r.clip.name} (${r.clip.duration.toFixed(2)} s) t=${(((r.time % r.clip.duration) + r.clip.duration) % r.clip.duration).toFixed(2)}` : `graph: ${DRIVES[st.drive].name} ${driveSpeed().toFixed(2)} m/s${st.path === 1 ? ' (circle)' : ''}${st.deliberate > 0 ? ` · deliberate ${st.deliberate.toFixed(2)}` : ''} · upper ${UPPERS[st.upper].mode} · phase ${char.anim.phase.toFixed(2)}`;
      lines.push(`${animLine} · stance ${STANCES[st.stance]}${st.lockpick ? ' · lockpick' : ''}${kickFlashT > 0 ? ' · ▮▮ KICK IMPACT ▮▮' : ''}${relFlashT > 0 ? ' · ▮▮ THROW RELEASE ▮▮' : char.anim.inThrow ? ` · throw ${char.anim.throwTime.toFixed(2)}s` : ''} · aim pitch ${st.aimPitchDeg}° · body yaw ${Math.round(wrapAngle(char.bodyYaw) / DEG)}°${st.paused ? ' · PAUSED' : ''}`);
