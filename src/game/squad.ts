...
import { goTo, followPath, attachFlashlight, hearingCheck, canSee, sightGain, bodyInView, escalationOf, doorTamper, nearKnownBody } from './guards';
...
export function runGuardScript(g: Game, gd: Guard, dt: number) {
  ...
  c.anim.speed = moveSpeed; c.anim.upper = s.upper ?? 'aim'; c.anim.crouchTarget = s.crouch ? 1 : 0;
  c.anim.stance = s.stance ?? 'none'; c.anim.stackSide = s.stackSide ?? 1; c.anim.lookYawExtra = 0;   // (the AI's leader-glance / wait-sweep neck twist must not freeze on him for the length of the script)
  c.update(dt);
...
export function endGuardScript(gd: Guard, state: 'patrol' | 'suspicious' | 'alert' | 'search' = 'patrol') {
  ...
  gd.char.anim.stance = 'none'; gd.char.anim.crouchTarget = 0; gd.char.anim.reverse = false; gd.state = state; gd.path = []; gd.pathGoal = null;   // (the AI never plays the cycle backwards, so it never clears it either)
...
export class RoomClear {
   ...
   *  does not call the corner he lies in 'empty' (BODY_LINES: the room is still cleared and called, he is just not nothing) */
  private say(key: keyof RoomClearPlan['lines'], gd: Guard, radio = false) {
    if (!this.once(key)) return;
    const grim = BODY_LINES[key]; this.g.say(gd, grim && this.bodyHere(gd) ? grim : this.plan.lines[key], radio);
  }
  ...
  private bodyHere(gd: Guard): boolean {
    const g = this.g, B = nearKnownBody(g, gd.char.pos, 3.5); if (!B) return false;
    const h = B.char.bones.hips ?? B.char.pos; if (this.plan.bounds && !inBounds([h[0], 0, h[2]], this.plan.bounds, 0.3)) return false;
    return !g.col.segmentBlocked(gd.char.bones.head ?? v3.add(gd.char.pos, [0, 1.65, 0]), [h[0], h[1] + 0.3, h[2]]);
  }
...
const STACK_CAP = 6;
/** what a man says instead of the plan's line when a found colleague lies within a few metres of him as he says it (RoomClear.say) */
