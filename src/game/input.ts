...
export class Input {
  ...
  private ctrlSolo = false;        // a Control press with nothing else pressed while it was down → 'CtrlTap' on release (crouch), so Ctrl chords never crouch
  ...
  constructor(private el: HTMLElement) {
    ...
    window.addEventListener('keydown', e => {
      ...
      if (e.code === 'ControlLeft' || e.code === 'ControlRight') this.ctrlSolo = !e.repeat || this.ctrlSolo; else if (e.ctrlKey) this.ctrlSolo = false;   // Ctrl+anything is a chord, not a crouch
    ...
    window.addEventListener('keyup', e => { this.keys.delete(e.code); if ((e.code === 'ControlLeft' || e.code === 'ControlRight') && this.ctrlSolo && !this.lockExcept) { this.pressed.add('CtrlTap'); } if (e.code === 'ControlLeft' || e.code === 'ControlRight') this.ctrlSolo = false; if (!typingTarget(e) && e.code !== 'Escape' && !e.metaKey) e.stopPropagation(); if (this.sink && e.target === this.sink) this.sink.value = ''; }, { capture: true });
    window.addEventListener('blur', () => { this.keys.clear(); this.buttons = 0; });
    el.addEventListener('mousemove', e => { if (this.lockExcept) return; const r = el.getBoundingClientRect(); this.mouseX = e.clientX - r.left; this.mouseY = e.clientY - r.top; });
    el.addEventListener('mousedown', e => { e.preventDefault(); this.focusGame(); this.ctrlSolo = false; if (this.lockExcept) { this.onLockedInput?.(); return; } if (this.suppressClick) { this.suppressClick = false; return; } this.buttons |= 1 << e.button; this.clicked |= 1 << e.button; });
    window.addEventListener('mouseup', e => { if (this.lockExcept) return; this.buttons &= ~(1 << e.button); });
    el.addEventListener('contextmenu', e => e.preventDefault());
    el.addEventListener('wheel', e => { e.preventDefault(); if (this.lockExcept) return; this.wheel += e.deltaY; }, { passive: false });
  }
  ...
  endFrame() { this.pressed.clear(); this.clicked = 0; this.wheel = 0; }
}
