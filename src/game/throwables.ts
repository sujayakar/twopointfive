  ...
  boxes(out: Box[]) {
    for (const it of this.items) {
      if (it.kind === 'smoke' || it.kind === 'flash') {
        ...
        if (standing) {
          out.push(makeBox({ c: [it.pos[0], it.pos[1] + 0.015, it.pos[2]], h: [0.032, 0.07, 0.032], yaw: it.yaw, albedo: body, flags: BoxFlag.Dynamic }));
          out.push(makeBox({ c: [it.pos[0], it.pos[1] + 0.06, it.pos[2]], h: [0.034, 0.012, 0.034], yaw: it.yaw, albedo: band, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));
        } else {   // in the air / skidding: the can lies along its local z and flips end over end (Box.rot: yaw, then tumble about the across axis)
          const rot = quat.mul(quat.yaw(it.yaw), quat.axisAngle([1, 0, 0], it.tumble));
          out.push(makeBox({ c: v3.copy(it.pos), h: [0.032, 0.032, 0.07], yaw: it.yaw, rot, albedo: body, flags: BoxFlag.Dynamic }));
          out.push(makeBox({ c: v3.add(it.pos, quat.rotate(rot, [0, 0, 0.045])), h: [0.034, 0.034, 0.012], yaw: it.yaw, rot, albedo: band, flags: BoxFlag.Dynamic | BoxFlag.NoShadow }));
        }
