...
(`Space` acts on the highlighted one, `↑`/`↓` pick when there are several); the pistol aims itself a little (the
...
**Grab.** From behind, a `Space` tap is still the silent takedown; *holding* it takes the man instead — an arm
across his throat if your hands are empty, the Five-seveN along his temple if it is drawn. The pair moves as one
at a creep and gives to walls and door leaves as one; walk him into a door his key fits and it opens. He is deaf,
blind and off the radio net while you have him, which the next radio check will notice. `Space` asks him a
question and he answers **from what he actually perceived tonight** — a man who never knew you were there is
...
plausibly know. Hold `Space` to finish it — a quiet choke-out with the arm, a quick loud pistol-whip with the gun,
both leaving him breathing (the men who find him say so, and the debrief counts knock-outs apart from kills) — or
`E` to shove him away, which makes him the best witness on the floor. With the gun at his head a click fires past
his ear at whatever the reticle has snapped to.
...
| mouse | aim — the reticle **snaps** to the nearest light or a man's chest within a couple of centimetres of screen · `LMB` fire (pistol out) / throw (slots 2, 3 — arc shown) / **melee from behind when the hands are empty** · `RMB` OCP the light the reticle is on (or the torch of the man under it) for a few seconds |
| `Space` | the highlighted interaction in the **top-left panel** (`↑` / `↓` when several are near): a door — tap to push it open / pull it shut, hold to crack a closed one silently, or if it is locked against you hold to pick it, tap standing (or sprint into it) to kick it in loud, tap crouched just to try it · a silent takedown from behind · drag / drop a body · take the magazine from a dropped pistol · hold to pull the drive |
| `E` · `Q` | holster / draw the selected item · press your back to the wall beside you — sidle along it, stop and peek at its end; `Q` again, pushing away or `Shift` lets go |
| `1` `2` `3` · `G` | Five-seveN · smoke canister (arc preview, bounces, 1.3 s fuse) · stun canister (bang, flash, fragments; every guard with line of sight is dazzled) · quick-throw the last canister |
| `R` `L` `N` | reload — empties hit the floor, partials go back to storage, the fullest spare goes in · pistol light · night vision |
...
| `P` · `←` `→` | hands-off showcase tour · step between its stops (or click the timeline along the bottom) |
| `Esc` · `Tab` · `` ` `` · `[` `]` | pause / settings / controls · the tweak panel (search box + an "all knobs" tier) · cycle debug views · exposure |
...
the chamber), spare magazines drawn by fill, canisters, gadget states and OCP charge; top-left, the interaction panel lists what `Space` can do here.
