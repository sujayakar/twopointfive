...
- **Reconnection shift.** Replace the first vertices and reconnect directly to x₂; reuse [x₂ … x_D] verbatim. Cheap, one shadow ray, constant storage. Fails badly when x₁ or x₂ is specular (shifted path contribution goes to ~0), and fails when the reconnection segment length differs wildly from the original (geometry term ratio explodes). This is what ReSTIR GI uses.
